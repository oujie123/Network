...
import com.gacrnd.gcs.okhttp.okhttp3.internal.connection.RouteDatabase;
...
public final class ConnectionPool {
    ...
    /**
     * todo 最多保存 5个 处于空闲状态的连接，连接的默认保活时间为 5分钟(移动联动5分钟断，电信28分钟断)
    ...
    RealConnection get(Address address, StreamAllocation streamAllocation, Route route) {
        ...
        for (RealConnection connection : connections) {
            ...
            if (connection.isEligible(address, route)) {
                //  todo 如果可以复用，则创建一个弱引用加入到connection的分配流list中
    ...
    long cleanup(long now) {
        ...
        synchronized (this) {
            ...
            //todo 超过了保活时间(5分钟) 或者池内空闲连接数量超过了(5个) 马上移除，然后返回0，表示不等待，马上再次检查清理
