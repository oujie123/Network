...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        while (true) {
            // todo 如果调用realcall的cancel在执行这个之前，则取消此次请求，并且关闭socket，如果在请求执行之后，则只是用户收不到请求
            if (canceled) {
            ...
            try {
                ...
                // todo 此时请求还没有发送，但是因为一些原因路由失败了，需要考虑重试，具体重试条件，在recover中
                if (!recover(e.getLastConnectException(), streamAllocation, false, request)) {
                    throw e.getLastConnectException();
                }
                releaseConnection = false;
                // todo 如果有重试，直接continue，while会继续重连
                continue;
            } catch (IOException e) {
                ...
                //todo 不是前两种的失败，那直接关闭清理所有资源,或者请求成功了，直接释放资源
            ...
            //todo 处理3和4xx的一些状态码，如301 302重定向。如果followUp是null 就直接返回响应结果了。
    ...
    private boolean recover(IOException e, StreamAllocation streamAllocation,
        ...
        //todo 3、判断是不是属于重试的异常，以下异常是致命的不重试，具体看isRecoverable中
    ...
    private boolean isRecoverable(IOException e, boolean requestSendStarted) {
        ...
        // todo 协议异常，错误码204 205。响应体body不为0，在CallServerInterceptor中抛出
        ...
        // todo 如果请求还没有发出，拦截链被中断，但是又有一个路由连接超时，此时是可能在连上路由的，所以返回true请求重试
    ...
    /**
     * todo 跟踪请求，这方法将增加认证头，跟踪重定向，或者处理客户端请求超时
     *
    ...
    private Request followUpRequest(Response userResponse, Route route) throws IOException {
        ...
        switch (responseCode) {
            case HTTP_PROXY_AUTH:
                // todo 如果返回407 说明代理需要认证，需要传入身份校验
                Proxy selectedProxy = route != null
                        ? route.proxy()
                        : client.proxy();
                // todo 如果不是HTTP代理,直接抛异常
            ...
            case HTTP_UNAUTHORIZED:
                // todo 处理401错误码，服务器需要授权，某些接口需要登录之后才能使用，目前基本都使用token来作为授权了。
            ...
            case HTTP_TEMP_REDIRECT:
                ...
                // todo 处理300-303、307、308状态码，如果不是GET和HEAD请求，直接返回
            ...
            case HTTP_CLIENT_TIMEOUT:
                ...
                // todo 处理408请求，向服务器发送请求，当发完请求头之后，服务器迟迟没有收到请求体，服务器就会返回408，通知客户端重新发送
                ...
                // todo 如果还有上一个请求且也是408，那就直接放弃，不试了
                ...
                // todo 如果响应头中有Retry-After，让客户端多久之后再重试一次，如果不是0秒，那就抛给用户自己读取重试时间，然后重试
